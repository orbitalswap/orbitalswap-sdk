import { Token, WNATIVE, ChainId, Pair, TokenAmount, Route } from '../src'
...
describe('Route', () => {
  ...
  const weth = WNATIVE[ChainId.BSC_MAINNET]
  ...
  it('supports ether input', () => {
    const route = new Route([pair_0_weth], WNATIVE[ChainId.BSC_MAINNET])
    expect(route.pairs).toEqual([pair_0_weth])
    expect(route.input).toEqual(WNATIVE[ChainId.BSC_MAINNET])
    expect(route.output).toEqual(token0)
  })

  it('supports ether output', () => {
    const route = new Route([pair_0_weth], token0, WNATIVE[ChainId.BSC_MAINNET])
    expect(route.pairs).toEqual([pair_0_weth])
    expect(route.input).toEqual(token0)
    expect(route.output).toEqual(WNATIVE[ChainId.BSC_MAINNET])
  })
})
