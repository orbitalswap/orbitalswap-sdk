...
describe('Router', () => {
  ...
  const pair_weth_0 = new Pair(new TokenAmount(WNATIVE[ChainId.BSC_MAINNET], '1000'), new TokenAmount(token0, '1000'))
    ...
    describe('exact in', () => {
      it('ether to token1', () => {
        const result = Router.swapCallParameters(
          Trade.exactIn(
            new Route([pair_weth_0, pair_0_1], NATIVE_CURRENCIES[ChainId.BSC_MAINNET], token1),
            new CurrencyAmount(NATIVE_CURRENCIES[ChainId.BSC_MAINNET], JSBI.BigInt(100))
          ),
        ...
        expect(result.args.slice(0, -1)).toEqual([
          '0x51',
          [WNATIVE[ChainId.BSC_MAINNET].address, token0.address, token1.address],
      ...
      it('token1 to ether', () => {
        const result = Router.swapCallParameters(
          Trade.exactIn(
            new Route([pair_0_1, pair_weth_0], token1, NATIVE_CURRENCIES[ChainId.BSC_MAINNET]),
        ...
        expect(result.args.slice(0, -1)).toEqual([
          '0x64',
          '0x51',
          [token1.address, token0.address, WNATIVE[ChainId.BSC_MAINNET].address],
          '0x0000000000000000000000000000000000000004'
        ])
