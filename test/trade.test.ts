...
import { ChainId, CurrencyAmount, Pair, Percent, Route, Token, TokenAmount, Trade, TradeType, WNATIVE } from '../src'
...
describe('Trade', () => {
  ...
  const pair_weth_0 = new Pair(
    new TokenAmount(WNATIVE[ChainId.BSC_MAINNET], JSBI.BigInt(1000)),
  ...
  it('can be constructed with ETHER as input', () => {
    const trade = new Trade(
      new Route([pair_weth_0], WNATIVE[ChainId.BSC_MAINNET]),
      new CurrencyAmount(WNATIVE[ChainId.BSC_MAINNET], JSBI.BigInt(100)),
      TradeType.EXACT_INPUT
    )
    expect(trade.inputAmount.currency).toEqual(WNATIVE[ChainId.BSC_MAINNET])
    expect(trade.outputAmount.currency).toEqual(token0)
  })
  it('can be constructed with ETHER as input for exact output', () => {
    const trade = new Trade(
      new Route([pair_weth_0], WNATIVE[ChainId.BSC_MAINNET], token0),
  ...
  it('can be constructed with ETHER as output', () => {
    const trade = new Trade(
      new Route([pair_weth_0], token0, WNATIVE[ChainId.BSC_MAINNET]),
      new CurrencyAmount(WNATIVE[ChainId.BSC_MAINNET], JSBI.BigInt(100)),
      TradeType.EXACT_OUTPUT
    )
    expect(trade.inputAmount.currency).toEqual(token0)
    expect(trade.outputAmount.currency).toEqual(WNATIVE[ChainId.BSC_MAINNET])
  })
  ...
  describe('#bestTradeExactIn', () => {
    ...
    it('works for ETHER currency input', () => {
      ...
      expect(result).toHaveLength(2)
      expect(result[0].inputAmount.currency).toEqual(WNATIVE[ChainId.BSC_MAINNET])
      expect(result[0].route.path).toEqual([WNATIVE[ChainId.BSC_MAINNET], token0, token1, token3])
      expect(result[0].outputAmount.currency).toEqual(token3)
      expect(result[1].inputAmount.currency).toEqual(WNATIVE[ChainId.BSC_MAINNET])
      expect(result[1].route.path).toEqual([WNATIVE[ChainId.BSC_MAINNET], token0, token3])
    ...
    it('works for ETHER currency output', () => {
      const result = Trade.bestTradeExactIn(
        ...
        WNATIVE[ChainId.BSC_MAINNET]
      )
      expect(result).toHaveLength(2)
      expect(result[0].inputAmount.currency).toEqual(token3)
      expect(result[0].route.path).toEqual([token3, token0, WNATIVE[ChainId.BSC_MAINNET]])
      expect(result[0].outputAmount.currency).toEqual(WNATIVE[ChainId.BSC_MAINNET])
      expect(result[1].inputAmount.currency).toEqual(token3)
      expect(result[1].route.path).toEqual([token3, token1, token0, WNATIVE[ChainId.BSC_MAINNET]])
      expect(result[1].outputAmount.currency).toEqual(WNATIVE[ChainId.BSC_MAINNET])
    })
  })
